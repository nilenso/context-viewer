...
describe("mapComponentsToIds", () => {
  ...
  it("matches recording [33]: maps 22 parts to components", async () => {
    ...
    const { mapping: result } = await mapComponentsToIds(pc1Conversation, components, testConfig);
  ...
  it("merges multiple batch results", async () => {
    ...
    const { mapping: result } = await mapComponentsToIds(conv, ["comp_a", "comp_b"], testConfig);
  ...
  it("returns {} on invalid AI response", async () => {
    ...
    const { mapping: result } = await mapComponentsToIds(pc1Conversation, ["a"], testConfig);
