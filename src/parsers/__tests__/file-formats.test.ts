...
import { parseFileContent, SUPPORTED_EXTENSIONS, createFileValidator } from "@/parsers/file-formats";
...
describe("createFileValidator", () => {
  const validate = createFileValidator();

  it("accepts .jsonl", () => {
    expect(validate(new File([""], "test.jsonl"))).toBeNull();
  });

  it("rejects .csv", () => {
    const result = validate(new File([""], "test.csv"));
    expect(result).not.toBeNull();
    expect(result!.code).toBe("file-invalid-type");
  });
});
