...
describe("segmentConversation", () => {
  ...
  it("respects custom segmentation threshold", async () => {
    ...
    await segmentConversation(conversation);
    expect(generateText).not.toHaveBeenCalled();

    // Custom threshold (100) → triggers segmentation
    // Matches recording [46] pattern: segmentationThreshold in args
    await segmentConversation(conversation, undefined, undefined, undefined, 100);
    expect(generateText).toHaveBeenCalledOnce();
  });
  ...
  it("reports progress", async () => {
    ...
    await segmentConversation(conversation, (done, total) => progress.push([done, total]));
