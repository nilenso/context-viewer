...
import { PC1_COLORS, SEG1_COMPONENTS, SEG1_COLORS } from "@/__tests__/recording-fixtures";
...
describe("assignComponentColors", () => {
  ...
  it("matches recording [35]: assigns colors to 51 components", async () => {
    ...
    const result = await assignComponentColors(components, testConfig);
  ...
  it("matches recording [40]: assigns colors to 49 segment-1 components", async () => {
    ...
    const result = await assignComponentColors(SEG1_COMPONENTS, testConfig);
  ...
  it("uses preset colors without AI", async () => {
    ...
    const result = await assignComponentColors(["A", "B"], testConfig, undefined, presetColors);
  ...
  it("returns {} on AI error", async () => {
    ...
    expect(await assignComponentColors(["A"], testConfig)).toEqual({});
  });
  ...
  it("strips '- ' prefix and description suffixes from AI keys", async () => {
    ...
    const result = await assignComponentColors(["planning", "coding"], testConfig);
...
describe("colorForDimension", () => {
  ...
  it("uses preset colors when provided", async () => {
    ...
    const result = await colorForDimension(dim, testConfig, undefined, presetColors);
