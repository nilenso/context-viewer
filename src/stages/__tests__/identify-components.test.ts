...
import { SEG1_COMPONENTS } from "@/__tests__/recording-fixtures";
...
describe("identifyComponents", () => {
  ...
  it("matches recording [26]: parses 49 components from AI for segment-1", async () => {
    ...
    const result = await identifyComponents(minimalConv, testConfig);
  ...
  it("returns [] when AI returns no JSON array", async () => {
    ...
    expect(await identifyComponents(minimalConv, testConfig)).toEqual([]);
  });
